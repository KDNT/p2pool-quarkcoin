...
nets = dict(
    ...
    Moleculecoin=math.Object(
        P2P_PREFIX='fea503dd'.decode('hex'),
        P2P_PORT=11879,
        ADDRESS_VERSION=58,
        RPC_PORT=18474,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'Moleculecoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 2048*100000000 >> (height + 1)//60480,
        BLOCKHASH_FUNC=lambda data: pack.IntType(256).unpack(__import__('Molecule_hash').getPoWHash(data)),
        POW_FUNC=lambda data: pack.IntType(256).unpack(__import__('Molecule_hash').getPoWHash(data)),
        BLOCK_PERIOD=30, # s
        SYMBOL='QRK',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Moleculecoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Moleculecoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.Moleculecoin'), 'Moleculecoin.conf'),
